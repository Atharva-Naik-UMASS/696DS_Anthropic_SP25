...
#     return train_file_path, args.generated_file_path, train_col, args.generated_col, args.label_col, args.train_legend, args.label_legend


# def plot_binned_line_graph(train_df, generated_df, train_col, generated_col, label_col, train_legend, label_legend):
...
#     print(f"Avg train_len: {train_df['train_len'].mean()} | gen: {generated_df['generated_len'].mean()} | label: {generated_df['label_len'].mean()}")
...
#     plt.title(f"Sentence Length Distribution (Binned)\nTraining: {train_legend} vs Generated vs Testing: {label_legend}", fontsize=14)
#     plt.tight_layout()
...
# def plot_density_graph(train_df, generated_df, train_col, generated_col, label_col, train_legend, label_legend):
#     train_df["train_len"] = train_df[train_col].astype(str).apply(len)
#     generated_df["generated_len"] = generated_df[generated_col].astype(str).apply(len)
#     generated_df["label_len"] = generated_df[label_col].astype(str).apply(len)
...
#     sns.kdeplot(train_df["train_len"], label=train_legend, color="blue", linewidth=2.0, ax=ax1)
#     ax1.set_xscale("log")
#     ax1.set_xlabel("Character Length", fontsize=12)
...
#     ax2 = ax1.twinx()
#     sns.kdeplot(generated_df["generated_len"], label="Generated", color="green", 
#                 linestyle="--", linewidth=2.0, ax=ax2)
#     sns.kdeplot(generated_df["label_len"], label=f"{label_legend} Label", 
#                 color="red", linestyle="-.", linewidth=2.0, ax=ax2)
#     ax2.set_ylabel("Density (Generated/Label)", fontsize=12)
...
#     plt.title(f"Sentence Length Distribution (Smoothed KDE)\n{train_legend} vs Generated vs {label_legend}", fontsize=14)
#     plt.tight_layout()

#     output_path = f"sentence_length_density_kde_dual_axis_{train_legend}_{label_legend}.png"
#     plt.savefig(output_path, dpi=300, bbox_inches="tight")
#     plt.show()
...
# #     sns.kdeplot(train_df["train_len"], label=train_legend, color="blue", linewidth=2.0)
# #     sns.kdeplot(generated_df["generated_len"], label="Generated", color="green", linestyle="--", linewidth=2.0)
# #     sns.kdeplot(generated_df["label_len"], label=f"{label_legend} Label", color="red", linestyle="-.", linewidth=2.0)

# #     ax.set_xscale("log")  # Comment out if log scale isn't helpful
...
# #     output_path = f"sentence_length_density_kde_{train_legend}_{label_legend}.png"
# #     plt.savefig(output_path, dpi=300, bbox_inches="tight")
# #     plt.show()
# #     print(f"[Saved] {output_path}")
...
# def main():
#     train_file_path, generated_file_path, train_col, generated_col, label_col, train_legend, label_legend = get_user_inputs()
    
#     train_df = pd.read_csv(train_file_path)
#     generated_df = pd.read_csv(generated_file_path)

#     plot_binned_line_graph(train_df, generated_df, train_col, generated_col, label_col, train_legend, label_legend)
#     plot_density_graph(train_df, generated_df, train_col, generated_col, label_col, train_legend, label_legend)
...
def plot_density_graph(train_df, generated_df, base_gen_df, train_col, generated_col, 
    ...
    # sns.kdeplot(generated_df["label_len"], label=f"{label_legend} Label", 
    #             color="red", linestyle="-.", linewidth=2.0, ax=ax2)
    ax2.set_ylabel("Density (SFT Model/Base Output)", fontsize=12)
